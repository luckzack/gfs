...
func parseFlags() error {
	...
	gcfg.Title = ":sweat_smile: %v files"
	gcfg.TitleIp = ""
	...
	kingpin.Flag("ip", "set to empty to disable it").StringVar(&gcfg.TitleIp)
	kingpin.Flag("namespace", "enabled namespace mode").BoolVar(&gcfg.Namespace)
...
func main() {
	...
	if len(gcfg.TitleIp) > 7 {
		gcfg.Title = fmt.Sprintf(gcfg.Title, gcfg.TitleIp)
	} else {
		hostname, _ := os.Hostname()
		gcfg.Title = fmt.Sprintf(gcfg.Title, hostname)
	}
	...
	//
	if gcfg.Namespace {
		middleware := basicauth.New("MyRealm", namespace.BasicAuth())
		hdlr = middleware(hdlr)
	} else {
